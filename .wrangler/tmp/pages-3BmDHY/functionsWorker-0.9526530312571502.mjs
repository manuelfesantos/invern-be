// ../.wrangler/tmp/bundle-RgYhux/checked-fetch.js
...
var onRequest3 = async (context) => {
  ...
  try {
    const body = bodySchema2.parse(await context.request.json());
    ...
    const iv = encode("ola");
    console.log("Setup key and iv", keyPair, iv);
    const encryptedPassword = decode(
    ...
    const db = context.env.INVERN_DB;
    console.log("db", db);
    await db.prepare("INSERT INTO user (id, username, password) VALUES (?, ?)").bind(crypto.randomUUID(), body.username, encryptedPassword).run();
    console.log("inserted");
    const user = await db.prepare("SELECT * FROM user WHERE username = $1").bind(body.username).first();
...
// ../.wrangler/tmp/pages-3BmDHY/functionsRoutes-0.16904130368903614.mjs
var routes = [
  {
    routePath: "/hasher",
    mountPath: "/",
...
// ../.wrangler/tmp/bundle-RgYhux/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-RgYhux/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9526530312571502.mjs.map
