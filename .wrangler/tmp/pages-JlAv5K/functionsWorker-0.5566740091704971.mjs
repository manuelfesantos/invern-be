// ../.wrangler/tmp/bundle-3xtTOx/checked-fetch.js
...
// ../libs/utils/crypto.ts
var hash = async (password, id) => {
  const encodedText = encode(password + id);
  const hash2 = await crypto.subtle.digest("SHA-256", encodedText);
  return decode(hash2);
};
...
// ../libs/entities/user/roles.ts
var rolesSchema = z.enum(["ADMIN", "USER"]);
var RolesEnum = rolesSchema.enum;

// ../libs/entities/product/product-entity.ts
...
// ../libs/entities/cart/cart-entity.ts
var cartSchema = z.object({
  id: z.string({ required_error: "id is required" }).uuid({ message: "Invalid id" }),
  products: z.array(productSchema).default([])
});

// ../libs/entities/user/user-entity.ts
...
var onRequest2 = async (context) => {
  ...
  try {
    const body = bodySchema2.parse(await request.json());
    initDb(env.INVERN_DB);
...
var onRequest3 = async (context) => {
  const { request, env } = context;
  if (request.method !== "POST") {
    return Response.json({ error: "method not allowed" }, { status: 405 });
  }
  try {
    const body = bodySchema3.parse(await request.json());
...
var onRequest4 = async (context) => {
  ...
  const db2 = context.env.INVERN_DB;
  await db2.prepare(
    `INSERT INTO users (id, username, password) VALUES ('124', 'John Doe', 'password')`
  ).run();
  return new Response("Hello!");
};

// users/index.ts
var onRequest5 = async (context) => {
  const { request, env } = context;
...
// ../libs/utils/constants.ts
var messages = {
  hello: "Hello World!"
};

// index.ts
var onRequest6 = async (context) => {
...
// ../.wrangler/tmp/bundle-3xtTOx/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-3xtTOx/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5566740091704971.mjs.map
